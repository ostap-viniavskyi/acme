...
class TrainingState(NamedTuple):
    ...
    action_pred_params: networks_lib.Params
    opt_state: optax.OptState
    action_pred_opt_state: optax.OptState
    steps: int
    random_key: networks_lib.PRNGKey


class NGULearner(acme.Learner):
    """NGU learner."""

    def __init__(self,
                 unroll: networks_lib.FeedForwardNetwork,
                 initial_state: networks_lib.FeedForwardNetwork,
                 action_pred: networks_lib.FeedForwardNetwork,
                 batch_size: int,
                 random_key: networks_lib.PRNGKey,
                 burn_in_length: int,
                 discount: float,
                 ...
                 action_pred_optimizer: optax.GradientTransformation,
                 action_pred_clipping_factor: int,
                 bootstrap_n: int = 5,
        ...
        def action_pred_loss(
                action_pred_params: networks_lib.Params,
                key_grad: networks_lib.PRNGKey,
                sample: reverb.ReplaySample
        ) -> jnp.ndarray:
            data = utils.batch_to_sequence(sample.data)  # [T, B, ...]

            observation_t = data.observation[-action_pred_clipping_factor:]  # [T1, B, ...]
            observation_tm1 = data.observation[-action_pred_clipping_factor - 1:-1]  # [T1, B, ...]
            action_tm1 = data.action[-action_pred_clipping_factor:]

            seq_length, batch_size = action_tm1.shape
            action_tm1_pred_logits = action_pred.apply(action_pred_params, key_grad,  # [T1, B, ...]
                                                       observation_tm1,
                                                       observation_t)
            ...
            return -jnp.sum(action_tm1_one_hot_labels * action_tm1_pred_log_prob) / (seq_length * batch_size)
        ...
        def sgd_step(
            ...
            grad_fn = jax.value_and_grad(action_pred_loss)
            key, key_grad = jax.random.split(key)
            action_pred_loss_value, action_pred_gradients = grad_fn(state.action_pred_params,
                                                                    key_grad,
                                                                    samples)

            # Average gradients over pmap replicas before optimizer update.
            action_pred_gradients = jax.lax.pmean(action_pred_gradients, _PMAP_AXIS_NAME)

            # Apply optimizer updates.
            action_pred_updates, action_pred_new_opt_state = action_pred_optimizer.update(action_pred_gradients,
                                                                                          state.action_pred_opt_state)
            action_pred_new_params = optax.apply_updates(state.action_pred_params, action_pred_updates)

            new_state = TrainingState(
                params=new_params,
                target_params=target_params,
                action_pred_params=action_pred_new_params,
                opt_state=new_opt_state,
                action_pred_opt_state=action_pred_new_opt_state,
                steps=steps,
                random_key=key)
            return new_state, priorities, {'loss': loss_value, 'action pred loss': action_pred_loss_value}
        ...
        random_key, key_init = jax.random.split(random_key)
        embedding_net_initial_params = embedding_net_action_pred.init(key_init)
        embedding_net_opt_state = embedding_net_optimizer.init(embedding_net_initial_params)

        state = TrainingState(
            params=initial_params,
            target_params=initial_params,
            embedding_net_params=embedding_net_initial_params,
            opt_state=opt_state,
            embedding_net_opt_state=embedding_net_opt_state,
            steps=jnp.array(0),
    ...
    def get_variables(self, names: List[str]) -> List[networks_lib.Params]:
        ...
        return [utils.get_from_first_device(self._state.params),
                utils.get_from_first_device(self._state.embedding_net_params)]
